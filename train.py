...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------#
    #   总训练世代epoch指的是遍历全部数据的总次数
    #   总训练步长total_step指的是梯度下降的总次数
    #   每个训练世代epoch包含若干训练步长step，每个训练步长step进行一次梯度下降。
    ...
    if True:
        ...
        if ngpus_per_node > 1:
            checkpoint = ParallelModelCheckpoint(model_body, os.path.join(log_dir,
                                                 ...
                                                 period=save_period)
            checkpoint_last = ParallelModelCheckpoint(model_body, os.path.join(log_dir, "last_epoch_weights.h5"),
                                                      monitor='val_loss', save_weights_only=True, save_best_only=False,
                                                      period=1)
            checkpoint_best = ParallelModelCheckpoint(model_body, os.path.join(log_dir, "best_epoch_weights.h5"),
            ...
            checkpoint_last = ModelCheckpoint(os.path.join(log_dir, "last_epoch_weights.h5"),
                                              monitor='val_loss', save_weights_only=True, save_best_only=False,
                                              period=1)
            checkpoint_best = ModelCheckpoint(os.path.join(log_dir, "best_epoch_weights.h5"),
